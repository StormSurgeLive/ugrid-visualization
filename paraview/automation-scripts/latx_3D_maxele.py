...
'''
PURPOSE:
Generate a figure of bathymetry/topography and water surface elevation from a zoomed-in 3D
perspective for an ADCIRC maxele.63 file
'''
...
ARGUMENTS:
    ...
    Argument 5: A string that defines the zoom area for the visualiation to focus on 
    Argument 6: A string that defines a contact email to be displayed in the visualization
...
'''
REQUIREMENTS:
This script will only work for visualizing a maxele.63.nc file (maximum water surface elevation
data on an unstructured mesh)

The input directory (argument 1) needs to contain a maxele.63.nc file, named exactly "maxele.63.nc"
and any alternate names will cause errors

The input directory also needs to contain an XDMF file associated with the maxele.63.nc, named 
exactly "maxele.63.nc.xmf"
...
'''
EXAMPLE USAGE:
Suppose you have saved the paths for the 6 arguments in shell variables ARG1, ARG2, ARG3, ARG4,
ARG5, and ARG6, and you have saved the path to your ParaView installation in shell variable PV_PATH

Then, usage of this script could look like this:
"$PV_PATH"/bin/pvpython latx_3D_maxele.py "$ARG1" "$ARG2" "$ARG3" "$ARG4" "$ARG5" "$ARG6"

After running this, you should see the file "latx_3D_maxele_img.png" in your output directory
'''
...
# Read maxele file
maxele63ncxmf = XDMFReader(registrationName='maxele.63.nc.xmf', FileNames=[path.join(pvpy_args[1],"maxele.63.nc.xmf")])
...
if (pvpy_args[5] == "New_Orleans_East"):
...
elif (pvpy_args[5] == "Lake_Pontchartrain"):
...
textEmail.Text = 'Contact Email: ' + pvpy_args[6] 
...
# Save figure
SaveScreenshot(path.join(pvpy_args[2],"latx_3D_maxele_img.png"), renderView1, ImageResolution=[1450, 778])
